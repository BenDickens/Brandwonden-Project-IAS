...
def vectorfield(w, t, p, params):
    ...
    ACH, ND_N , TGFB, FIBRO, COLL, PDGF = pre_vectorfield(w, params)
    ...
    step_max, r_NDN, r_ITM, ITMblood_peak, r_AP, rinduce, ITM_source_peak, ITM_source_width, mu_TGFB, mu_FIBRO,\
    mu_COLL, beta_TGFB_M_A, beta_PDGF_M_A, beta_PDGF_FIBRO, beta_TGFB_FIBRO, beta_FIBRO_COLL, inverse_max_M_A, \
    inverse_max_fibro, inverse_max_coll, growth_FIBRO = p
    ...
    f = [
        ...
        + (r_Nhomeo * (1. - N_R / Nmax) * N_R) * (int(not (N_R < 0))) + 3
        ,
        ...
        * (int(not (ITMblood < 0)))
        ...
        + (((Pmax_MR - Pmin_MR) * (CH / (CH + Keq_CH)) + Pmin_MR) * (Mmax - (M_A + M_R))) + 3,
        ...
        #FIBROBLASTS
        -mu_FIBRO * FIBRO + growth_FIBRO * prms.fibroblast_vs_TGFB(FIBRO)* FIBRO * (1- (M_A/2500 + FIBRO/1250 + COLL/3300)),


        #COLLAGEN

        - mu_COLL * COLL + beta_FIBRO_COLL * FIBRO * prms.collagen_vs_collagen_density(COLL)* prms.collagen_vs_TGFB(TGFB),
...
def pre_vectorfield(w, params):
    N_R, AP_Eblood, AP_Etissue, AP_Eliver, AP_Sblood, AP_Stissue, ITMb, ITM, M_R, M_A, CH, N_A, ND_A, ACH, ND_N, TGFB, FIBRO, COLL, PDGF = w
    if params['case'] == 0:
        N_R = AP_Eblood = AP_Etissue, AP_Eliver = AP_Sblood = AP_Stissue = M_R = M_A = CH = N_A = ND_A = ACH = ND_N = TGFB= FIBRO = COLL= PDGF = 0
    elif params['case'] == 1:
    ...
    return [N_R, AP_Eblood, AP_Etissue, AP_Eliver, AP_Sblood, AP_Stissue, ITMb, ITM, M_R, M_A, CH, N_A, ND_A, ACH, ND_N, TGFB, FIBRO, COLL, PDGF]


def post_vectorfield(f, w, params):
    _w = place_finder(w)
    N_R, AP_Eblood, AP_Etissue, AP_Eliver, AP_Sblood, AP_Stissue, ITMb, ITM, M_R, M_A, CH, N_A, ND_A, ACH, ND_N, TGFB, FIBRO , COLL, PDGF= _w
    if params['case'] == 0:
        f[N_R] = f[AP_Eblood] = f[AP_Etissue] = f[AP_Eliver] = f[AP_Sblood] = f[AP_Stissue] \
            = f[ITMb] = f[ITM] = f[M_R] = f[M_A] = f[CH] = f[N_A] = f[ND_A] = f[ACH] = f[ND_N] = f[TGFB] = f[FIBRO] = f[COLL] =f[PDGF]= 0
    elif params['case'] == 1:
...
def get_init(w0, params):
    ...
    Arguments:
        w0       :  vector of the initial conditions
    """
    N_R0, AP_Eb0, AP_Et0, AP_El0, AP_S0, AP_St0, ITMb0, ITM0, M_R0, M_A0, CH0, N_A0, ND_A0, ACH0, ND_N0, TGFB0, FIBRO0, COLL0, PDGF0= w0

    if params['case'] == 0:
        N_R0 = AP_Eb0 = AP_Et0 = AP_El0 = AP_S0 = AP_St0 = M_R0 = M_A0 = CH0 = N_A0 = ND_A0 = ACH0 = ND_N0 = TGFB0 = FIBRO0 = COLL0= PDGF0= 0
    elif params['case'] == 1:
        ...
        AP_S0 = AP_St0 = 0
    w0 = [N_R0, AP_Eb0, AP_Et0, AP_El0, AP_S0, AP_St0, ITMb0, ITM0, M_R0, M_A0, CH0, N_A0, ND_A0, ACH0, ND_N0, TGFB0, FIBRO0, COLL0, PDGF0]
    return w0
...
def get_header():
    header = ['N_R', 'AP_Eblood', 'AP_Etissue', 'AP_Eliver', 'AP_Sblood', 'AP_Stissue', 'ITMblood', 'ITMtissue',
              'M_R', 'M_A', 'CH', 'N_A', 'ND_A', 'ACH', 'ND_N', 'TGFB', 'FIBRO', 'COLL', 'PDGF']
    return header
...
def plot(wsol, df_AP_median, case_names, timein, t_interval, df_IL6, df_IL10, params):
    ...
    if case >= 1:
        ...
        plt.ylabel("concentration $(g/mm^3$)", fontsize=fs)
...
if __name__ == "__main__":
    ...
    beta_PDGF_M_A = prms.beta_PDGF_M_A
    beta_PDGF_FIBRO = prms.beta_PDGF_FIBRO
    beta_TGFB_FIBRO = prms.beta_TGFB_FIBRO
    beta_FIBRO_COLL = prms.beta_FIBRO_COLL

    inverse_max_M_A = prms.inverse_max_M_A
    inverse_max_fibro = prms.inverse_max_fibro
    inverse_max_coll = prms.inverse_max_coll

    growth_FIBRO = prms.growth_FIBRO
    ...
    for case in cases:
        ...
        p = [mu_ACH , mu_APE, mu_APS, mu_APSfast, mu_APSslow, mu_CH, mu_MA, mu_MR, mu_NA, mu_NDA, mu_NDN,
             ...
             r_NDN, r_ITM, r_ITMpeak, r_AP, rinduce, ITM_source_peak, ITM_source_width, mu_TGFB, mu_FIBRO, mu_COLL,
             beta_TGFB_M_A, beta_PDGF_M_A, beta_PDGF_FIBRO, beta_TGFB_FIBRO, beta_FIBRO_COLL, inverse_max_M_A,
             inverse_max_fibro, inverse_max_coll, growth_FIBRO ]

        w = [N_R0, AP_Eb0, AP_Et0, AP_El0, AP_S0, AP_St0, ITMblood0, ITMtissue0, M_R0, M_A0, CH0, N_A0, ND_A0, ACH0, ND_N0, TGFB0, FIBRO0, COLL0, PDGF0]
        params['case'] = case
